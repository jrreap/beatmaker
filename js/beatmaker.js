...
const INSTRUMENTS_ARRAY = ['synths','piano','organ','horn','guitar','flute','bass']
const BEAT_ARRAY=[1,1,1,1,1,1,1]
...
function generateWorkspace () {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    beatMatrix[i] = []
    audioMatrix[i] = []
    const symbol = $(`<div id='track${i}-symbol' class="col track-symbol">${INSTRUMENTS_ARRAY[i]}</div>`)
    row.append(symbol)
    ...
    for (let j = 0; j < colLimit; j++) {
      ...
      beatMatrix[i][j] = ''
      audioMatrix[i][j] = new Audio(`./Assets/audio/${INSTRUMENTS_ARRAY[i]}${BEAT_ARRAY[i]}.wav`)
...
function setSpaceInstrument (row, col, element, instrument) {
  if (beatMatrix[row][col] != '') {
    ...
    beatMatrix[row][col] = '';
    element.empty()
    // console.log(element.indexOf(instrument))
  }
  else {
    element.text('')
    element.append(INSTRUMENTS_ARRAY[row])

    beatMatrix[row][col] = INSTRUMENTS_ARRAY[row] + sampleIndex
    audioMatrix[row][col].play()
    // console.log(element)
  }
